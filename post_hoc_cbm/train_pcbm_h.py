...
def config():
    ...
    parser.add_argument("--finetuning-path", required=False, default="", type=str, help="Specify if the model from the pcbm path must be updated by a finetuned version.")
    parser.add_argument("--finetuned-clip-path", required=False, default="", type=str, help="Specify path to load finetuned clip as a backbone.")    
...
if __name__ == "__main__":    
    args = config()    
    # Load the PCBM
    posthoc_layer = torch.load(args.pcbm_path) # underlying PCBM, might need to be updated if there is a finetuning path 
    args.backbone_name = posthoc_layer.backbone_name
    backbone, preprocess = get_model(args, backbone_name=args.backbone_name)
    backbone = backbone.to(args.device)

    if args.finetuned_clip_path != "":
        checkpoint = torch.load(args.finetuned_clip_path, map_location = args.device)
        clip_model_state_dict = checkpoint['clip_model_state_dict']   
        backbone.load_state_dict(clip_model_state_dict)

    if args.finetuning_path != "":
        checkpoint = torch.load(args.finetuning_path, map_location = args.device)
        clip_model_state_dict = checkpoint['clip_model_state_dict']
        pcbm_model_state_dict = checkpoint['pcbm_model_state_dict']
        posthoc_layer.load_state_dict(pcbm_model_state_dict)
        backbone.load_state_dict(clip_model_state_dict)
