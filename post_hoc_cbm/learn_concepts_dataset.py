...
def config():
    ...
    parser.add_argument("--backbone-path", default="", type=str, help="Specify a path when working with finetuned clip")
    parser.add_argument("--dataset-name", default="cifar10", type=str)
    parser.add_argument("--out-dir", required=True, type=str)
    parser.add_argument("--adv-conc", required=False, default=False, type=bool, help="Set to True to learn the CAVs with adversarial training")
...
def main():
    ...
    if args.backbone_path != "":
        checkpoint = torch.load(args.backbone_path, map_location = args.device)
    ...
    for concept_name, loaders in concept_loaders.items():
        ...
        cav_info = learn_concept_bank(pos_loader, neg_loader, backbone, n_samples, args.C, device=args.device, adv=args.adv_conc)
